  ...
  <section class="workbench-shell">
    <div class="workbench-head">
      <div>
        ...
        <p class="workbench-sub">把数字孪生快照、eCASH、ICU 获得性衰弱、肺栓塞、长期随访、阈值审核、相似病例和运行状态都放到一个可见入口里。</p>
      </div>
    ...
    <div v-if="showOpsCards" class="ops-grid">
      <article class="ops-card ops-card--runtime">
        <div class="ops-title-row">
          <span class="ops-kicker">运行态</span>
          <strong>大模型运行态</strong>
        </div>
        <div class="ops-main">{{ runtime.detail || '当前辅助服务运行平稳。' }}</div>
...
<style scoped>
.workbench-shell { display: grid; gap: 12px; padding: 14px; border-radius: 18px; border: 1px solid rgba(71, 145, 191, 0.18); background: #FFFFFF; box-shadow: inset 0 1px 0 rgba(194, 236, 255, 0.05); }
...
.topic-card { padding: 12px; border-radius: 4px; border: 1px solid rgba(110, 231, 249, 0.12); background: #FFFFFF; text-align: left; color: inherit; cursor: pointer; transition: transform .18s ease, border-color .18s ease, box-shadow .18s ease; }
.topic-card:hover { transform: translateY(-2px); border-color: rgba(125, 211, 252, 0.34); box-shadow: 0 1px 2px rgba(0,0,0,.06); }
...
.ops-card { padding: 16px; border-radius: 18px; border: 1px solid rgba(93, 226, 231, 0.12); background: #FFFFFF; }
...
html[data-theme='light'] .ops-link { color: #15558D; }
...
html[data-theme='light'] .topic-card-title,
html[data-theme='light'] .ops-card strong { color: #1D2129; }
html[data-theme='light'] .topic-card-sub,
html[data-theme='light'] .topic-card-meta,
html[data-theme='light'] .ops-foot { color: #4E5969; }
html[data-theme='light'] .topic-card-main,
html[data-theme='light'] .ops-main { color: #1D2129; }
html[data-theme='light'] .topic-card-count {
  background: rgba(243,248,252,0.96);
  color: #47627e;
}
