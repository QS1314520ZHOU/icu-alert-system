...
import { buildDeviceMarkers, buildPatientOrganStateFromAlerts, normalizeBodyMapOrganKey } from '../../utils/bodyMap'
...
export function usePatientDetail() {
  ...
  function fmtBP(v: any) { const s = v?.nibp_sys, d = v?.nibp_dia; return s != null || d != null ? ${s ?? '—'}/ : '—' }
  ...
  function formatClinicalMeasure(value: any, unit = '', digits = 1) { const t = formatClinicalNumber(value, digits); return t === '—' ? t : ${t} }
  ...
  function formatCountdown(seconds?: number | null) { if (seconds == null) return '—'; const safe = Math.max(0, Math.floor(seconds)); const h = Math.floor(safe / 3600), m = Math.floor((safe % 3600) / 60), s = safe % 60; return h > 0 ? ${h}h m : ${m}m s }
  ...
  function handoffPlainText() { const s = aiHandoff.value || {}; return [Illness severity: , Patient summary: , Confidence: ].join('\n') }
  ...
  function aiRiskOrganRows(item: any) { return [] }
  function readTrendLegendSelection() {}
  const displayName = computed(() => patient.value?.name || patient.value?.hisName || '加载中...')
  const displaySubTitle = computed(() => { const bed = patient.value?.hisBed || patient.value?.bed || '--'; const g = patient.value?.genderText || patient.value?.hisSex || ''; const a = patient.value?.age || patient.value?.hisAge || ''; return ${bed}床 |  .trim() })
  ...
  const sepsisBundleTimelineText = computed(() => { const s = sepsisBundleStatusResolved.value; const started = s?.bundle_started_at ? fmtTime(s.bundle_started_at) : ''; const dl = s?.deadline_1h ? fmtTime(s.deadline_1h) : ''; if (s?.status === 'met' || s?.status === 'met_late') return \u8D77\u70B9 ; if (s?.status === 'pending' || s?.status === 'overdue_1h') return \u8D77\u70B9  \u00B7 1h\u622A\u6B62 ; return '\u672A\u89C1\u8BA1\u65F6\u8BB0\u5F55' })
  const sepsisBundleExtraText = computed(() => { const s = sepsisBundleStatusResolved.value; if (s?.status === 'pending') return \u5269\u4F59 ; if (s?.status === 'overdue_1h') return \u5DF2\u8D85\u65F6 ; return '' })
  ...
  // ===== LOAD FUNCTIONS =====
  async function loadAlerts() { const pid = route.params.id as string; if (!pid) return; try { const res = await getPatientAlerts(pid); alerts.value = res.data.records || []; const ids = alerts.value.map((i: any) => String(i?._id || '')).filter(Boolean).slice(0, 50); if (ids.length) postPatientAlertsViewed(pid, { alert_ids: ids, actor: getOperatorIdentity(), source: 'patient_detail' }).catch(() => undefined) } catch {} }
  async function loadClinicalSummary() { const pid = String(route.params.id || '').trim(); if (!pid) return; clinicalSummaryLoading.value = true; try { const res = await getPatientClinicalSummary(pid, { hours: 24 }); clinicalSummary.value = res.data?.data || null } catch { clinicalSummary.value = null } finally { clinicalSummaryLoading.value = false } }
  async function loadSepsisBundleStatus() { const pid = route.params.id as string; if (!pid) return; try { const res = await getPatientSepsisBundleStatus(pid); sepsisBundleStatus.value = res.data?.status || null; sepsisBundleNow.value = Date.now() } catch { sepsisBundleStatus.value = null } }
  async function loadWeaningStatus() { const pid = route.params.id as string; if (!pid) return; try { const res = await getPatientWeaningStatus(pid); weaningStatus.value = res.data?.status || null } catch { weaningStatus.value = null } }
  async function loadTrend() { const pid = route.params.id as string; if (!pid) return; try { const res = await getPatientVitalsTrend(pid, trendWindow.value); trendPoints.value = res.data.points || []; trendLoaded.value = true } catch {} }
  async function loadLabs() { const pid = route.params.id as string; if (!pid || labsLoaded.value) return; try { const res = await getPatientLabs(pid); labs.value = res.data.exams || []; labsLoaded.value = true } catch {} }
  async function loadDrugs() { const pid = route.params.id as string; if (!pid || drugsLoaded.value) return; try { const res = await getPatientDrugs(pid); drugs.value = res.data.records || []; drugsLoaded.value = true } catch {} }
  async function loadAssessments() { const pid = route.params.id as string; if (!pid || assessmentsLoaded.value) return; try { const res = await getPatientAssessments(pid); assessments.value = res.data.records || []; assessmentsLoaded.value = true } catch {} }
  async function loadClinicalTrialMatches() { if (trialMatchLoading.value) return; const pid = route.params.id as string; if (!pid) return; trialMatchLoading.value = true; try { const res = await getPatientTrialMatches(pid); trialMatches.value = res.data?.matches || [] } catch { trialMatchError.value = 'failed' } finally { trialMatchLoading.value = false } }
  async function loadSbtTimeline(force = false) { if (sbtTimelineLoading.value) return; if (sbtTimelineLoaded.value && !force) return; const pid = route.params.id as string; if (!pid) return; sbtTimelineLoading.value = true; try { const res = await getPatientWeaningTimeline(pid, 40); sbtTimelineSummary.value = res.data?.summary || null; sbtTimelineRecords.value = Array.isArray(res.data?.timeline) ? res.data.timeline : [] } catch (e: any) { sbtTimelineError.value = e?.response?.data?.message || 'failed' } finally { sbtTimelineLoading.value = false; sbtTimelineLoaded.value = true } }
  async function loadSimilarCaseReview(force = false) { if (similarCaseLoading.value) return; if (similarCaseLoaded.value && !force) return; const pid = route.params.id as string; if (!pid) return; similarCaseLoading.value = true; try { const res = await getPatientSimilarCaseOutcomes(pid, 10); similarCaseReview.value = res.data?.review || null } catch { similarCaseReview.value = { summary: { matched_cases: 0, degraded: true }, cases: [], historical_case_insight: { summary: 'failed', pattern_bullets: [] } } } finally { similarCaseLoading.value = false; similarCaseLoaded.value = true } }
  async function loadPersonalizedThresholds(force = false) { if (personalizedThresholdLoading.value) return; if (personalizedThresholdRecord.value && !force) return; const pid = route.params.id as string; if (!pid) return; personalizedThresholdLoading.value = true; try { const [l, h] = await Promise.all([getPatientPersonalizedThresholds(pid), getPatientPersonalizedThresholdHistory(pid, { limit: 6 })]); personalizedThresholdRecord.value = l.data?.record || null; personalizedThresholdHistory.value = Array.isArray(h.data?.rows) ? h.data.rows : [] } catch {} finally { personalizedThresholdLoading.value = false } }
  async function loadWaveform() { if (waveformLoading.value) return; const pid = route.params.id as string; if (!pid) return; waveformLoading.value = true; try { const cr = await getWaveformChannels(pid, { hours: 24 }); waveformChannels.value = Array.isArray(cr.data?.rows) ? cr.data.rows : []; if (!waveformSelectedChannel.value && waveformChannels.value.length) waveformSelectedChannel.value = String(waveformChannels.value[0]?.channel || ''); if (!waveformSelectedChannel.value) { waveformPoints.value = []; waveformQc.value = null; waveformEvents.value = []; return } const [seg, qc, ev] = await Promise.all([getWaveformSegments(pid, { channel: waveformSelectedChannel.value, hours: waveformHours.value, limit: 2000 }), getWaveformQuality(pid, { channel: waveformSelectedChannel.value, hours: waveformHours.value }), getWaveformEvents(pid, { channel: waveformSelectedChannel.value, hours: waveformHours.value })]); waveformPoints.value = Array.isArray(seg.data?.rows) ? seg.data.rows : []; waveformQc.value = qc.data?.qc || null; waveformEvents.value = Array.isArray(ev.data?.events) ? ev.data.events : [] } catch {} finally { waveformLoading.value = false } }
  async function loadAiLab() { if (aiLabLoading.value) return; const pid = route.params.id as string; if (!pid) return; aiLabLoading.value = true; try { const r = await getAiLabSummary(pid); aiLabSummary.value = stripModelThinking(r.data.summary || ''); aiLabError.value = formatAiError(r.data.error || '') } catch { aiLabError.value = 'failed' } finally { aiLabLoading.value = false } }
  async function loadAiRules() { if (aiRuleLoading.value) return; const pid = route.params.id as string; if (!pid) return; aiRuleLoading.value = true; try { const r = await getAiRuleRecommendations(pid); aiRulePayload.value = Array.isArray(r.data.recommendations) ? r.data.recommendations : null; aiRuleText.value = stripModelThinking(typeof r.data.raw_text === 'string' ? r.data.raw_text : JSON.stringify(r.data.recommendations || '', null, 2)); aiRuleError.value = formatAiError(r.data.error || '') } catch { aiRuleError.value = 'failed' } finally { aiRuleLoading.value = false } }
  async function loadAiRisk() { if (aiRiskLoading.value) return; const pid = route.params.id as string; if (!pid) return; aiRiskLoading.value = true; try { const r = await getAiRiskForecast(pid); aiRiskForecast.value = r.data || null; aiRiskText.value = stripModelThinking(r.data.risk_summary || ''); aiRiskError.value = formatAiError(r.data.error || '') } catch { aiRiskError.value = 'failed' } finally { aiRiskLoading.value = false } }
  async function loadIntegratedRisk(refresh = false) { if (integratedRiskLoading.value) return; const pid = route.params.id as string; if (!pid) return; integratedRiskLoading.value = true; try { const r = await getAiIntegratedRiskReport(pid, { refresh }); integratedRiskReport.value = r.data.report || null; integratedRiskError.value = formatAiError(r.data.error || '') } catch { integratedRiskError.value = 'failed' } finally { integratedRiskLoading.value = false } }
  async function loadMetabolicPhase(refresh = false) { if (metabolicPhaseLoading.value) return; const pid = route.params.id as string; if (!pid) return; metabolicPhaseLoading.value = true; try { const r = await getAiMetabolicPhase(pid, { refresh }); metabolicPhaseRecord.value = r.data.record || null } catch { metabolicPhaseRecord.value = { phase: 'insufficient_data', degraded: true } } finally { metabolicPhaseLoading.value = false } }
  async function loadBetaBlockerAdvisor(refresh = false) { if (betaBlockerAdvisorLoading.value) return; const pid = route.params.id as string; if (!pid) return; betaBlockerAdvisorLoading.value = true; try { const r = await getAiBetaBlockerAdvisor(pid, { refresh }); betaBlockerAdvisorRecord.value = r.data.record || null; betaBlockerAdvisorError.value = formatAiError(r.data.error || '') } catch { betaBlockerAdvisorError.value = 'failed' } finally { betaBlockerAdvisorLoading.value = false } }
  async function loadFibrinolysis(refresh = false) { if (fibrinolysisLoading.value) return; const pid = route.params.id as string; if (!pid) return; fibrinolysisLoading.value = true; try { const r = await getAiFibrinolysisMonitor(pid, { refresh }); fibrinolysisRecord.value = r.data.record || null } catch { fibrinolysisRecord.value = { score_type: 'fibrinolysis_monitor', assessment: { phenotype: 'insufficient_data' }, degraded: true } } finally { fibrinolysisLoading.value = false } }
  async function loadPronePosition(refresh = false) { if (pronePositionLoading.value) return; const pid = route.params.id as string; if (!pid) return; pronePositionLoading.value = true; try { const r = await getAiPronePositionMonitor(pid, { refresh }); pronePositionRecord.value = r.data.record || null } catch { pronePositionError.value = 'failed' } finally { pronePositionLoading.value = false } }
  async function loadPicsRisk(refresh = false) { if (picsRiskLoading.value) return; const pid = route.params.id as string; if (!pid) return; picsRiskLoading.value = true; try { const r = await getAiPicsRisk(pid, { refresh }); picsRiskRecord.value = r.data.record || null } catch { picsRiskError.value = 'failed' } finally { picsRiskLoading.value = false } }
  async function loadAiHandoff() { if (aiHandoffLoading.value) return; const pid = route.params.id as string; if (!pid) return; aiHandoffLoading.value = true; try { const r = await getPatientHandoffSummary(pid); aiHandoff.value = r.data.summary || null; aiHandoffError.value = formatAiError(r.data.error || '') } catch { aiHandoffError.value = 'failed' } finally { aiHandoffLoading.value = false } }
  ...
  watch(() => route.params.id, (next, prev) => { if (next && next !== prev) { resetDetailState(); void loadDetailPage() } })
  ...
  return {
    ...
    // Alerts
    alerts, alertTypeText, formatAlertValue, alertSeverityText, sortAlertsDesc,
    // Sepsis bundle
    ...
    // Weaning
    weaningAssessment, sbtAssessment, weaningRiskLabel, weaningRecommendationText,
    // SBT timeline
    ...
    // Alerts CRUD
    acknowledgeAlert, openEvidence, resetDetailState,
    // Load all
    loadDetailPage, loadAlerts,
    // Constants
