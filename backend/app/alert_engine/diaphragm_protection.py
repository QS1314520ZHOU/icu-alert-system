...
class DiaphragmProtectionMixin:
    ...
    async def scan_diaphragm_protection(self) -> None:
        ...
        for patient_doc in patients:
            ...
            if not active_bind:
                continue
            vent_days = await self._get_current_ventilation_days(pid_str, now) if hasattr(self, "_get_current_ventilation_days") else 0.0
            ...
            rr_measured = self._vent_param_priority(cap, ["rr_measured", "rr_set"], ["param_vent_resp", "param_HuXiPinLv"])
            rr_set = self._vent_param(cap, "rr_set", "param_HuXiPinLv")
            vte_ml = self._vent_param_priority(cap, ["vte", "vt_set"], ["param_vent_vt", "param_vent_set_vt"])
            ...
            weight_kg = self._get_patient_weight(patient_doc) or 70.0
            ...
            accessory = await self._get_accessory_muscle_sign(pid, now, hours=12) if hasattr(self, "_get_accessory_muscle_sign") else None
            drive = await self._latest_diaphragm_drive(pid, device_id)
