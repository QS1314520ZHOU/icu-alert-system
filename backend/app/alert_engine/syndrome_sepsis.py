...
class SepsisMixin:
    ...
    async def scan_sepsis(self) -> None:
        ...
        for patient_doc in patients:
            ...
            tracker = await self._start_or_refresh_sepsis_bundle_tracker(
                patient_doc=patient_doc,
                pid_str=pid_str,
                now=now,
            ...
            if not tracker:
                tracker = await self._get_active_sepsis_bundle_tracker(pid_str)
            triggered += await self._evaluate_sepsis_bundle_tracker(
                tracker=tracker,
                patient_doc=patient_doc,
                pid_str=pid_str,
                device_id=device_id,
