...
class BleedingScanner(BaseScanner):
    def __init__(self, engine) -> None:
        super().__init__(
            engine,
            ScannerSpec(
                name="bleeding",
                interval_key="bleeding",
    ...
    async def scan(self) -> None:
        ...
        for p in patients:
            ...
            hb_drop = await self.engine._get_hb_drop(his_pid, hours=24)
            if not hb_drop or hb_drop["drop"] < 20:
                continue

            pid_str = str(p.get("_id"))
            latest_vitals = await self.engine._get_latest_vitals_by_patient(p.get("_id"))
        ...
        if triggered > 0:
            self.engine._log_info("出血预警", triggered)
