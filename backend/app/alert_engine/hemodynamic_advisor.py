    ...
    async def scan_hemodynamic_advisor(self) -> None:
        ...
        for patient_doc in patients:
            ...
            if not pid:
                continue
            pid_str = str(pid)
            snapshot = await self._get_latest_param_snapshot_by_pid(pid, codes=ppv_codes + svv_codes)
            if not snapshot:
                continue
            ppv = None
            svv = None
            ...
            vte = self._vent_param_priority(vent_cap or {}, ["vte", "vt_set"], ["param_vent_vt", "param_vent_set_vt"]) if vent_cap else None
            rr_set = self._vent_param(vent_cap or {}, "rr_set", "param_HuXiPinLv") if vent_cap else None
            rr_measured = self._vent_param(vent_cap or {}, "rr_measured", "param_vent_resp") if vent_cap else None
            weight = self._get_patient_weight(patient_doc) or 70.0
            vt_ml_kg = round(vte / weight, 2) if (vte is not None and weight > 0) else None

            rhythm_text = ""
            rhythm_doc = await self.db.col("bedside").find_one(
            ...
            if not prerequisites_ok:
                ...
                if (ppv is not None and ppv > 15) and await self._has_recent_drug(pid, ["去甲肾上腺素", "norepinephrine"], hours=12):
                    severity = "high"
        ...
        if triggered > 0:
            self._log_info("血流动力学建议", triggered)
