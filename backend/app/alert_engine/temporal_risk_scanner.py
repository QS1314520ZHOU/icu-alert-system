...
class TemporalRiskScannerMixin:
    ...
    async def scan_temporal_risk(self) -> None:
        ...
        for patient_doc in patients:
            ...
            pid_str = str(pid)
            try:
                sequence, meta_features, context = await self._prepare_temporal_input(
                ...
                result = await asyncio.to_thread(
                    self._get_temporal_model_runtime().predict,
                    sequence=sequence,
            ...
            severity = self._risk_level_from_probability(prob_4h)
        ...
        if triggered > 0:
            self._log_info("时序风险扫描", triggered)
