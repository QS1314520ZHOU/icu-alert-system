...
class DicScanner(BaseScanner):
    ...
    async def scan(self) -> None:
        ...
        for p in patients:
            ...
            if not high_risk:
                recent_sepsis = await self.engine.db.col("alert_records").find_one({
                    "patient_id": pid_str,
            ...
            dic = await self.engine._calc_dic_score(his_pid)
            if not dic:
                continue
        ...
        if triggered > 0:
            self.engine._log_info("DIC预警", triggered)
