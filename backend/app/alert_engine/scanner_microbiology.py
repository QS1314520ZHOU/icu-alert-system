...
class MicrobiologyScanner(BaseScanner):
    ...
    async def scan(self) -> None:
        ...
        for patient_doc in patients:
            ...
            mdro_trigger_cfg = self.engine.config.yaml_cfg.get("alert_engine", {}).get("microbiology", {}).get("mdro_screening_trigger", {})
            mdro_trigger_cfg = mdro_trigger_cfg if isinstance(mdro_trigger_cfg, dict) else {}
            prior_mdro = await self.engine._latest_prior_mdro_alert(
                pid_str,
