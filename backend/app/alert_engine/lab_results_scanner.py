...
from app.utils.clinical import _eval_condition

from .scanners import BaseScanner, ScannerSpec


class LabResultsScanner(BaseScanner):
    def __init__(self, engine) -> None:
        super().__init__(
            engine,
            ScannerSpec(
                name="lab_results",
                interval_key="lab_results",
    ...
    async def scan(self) -> None:
        ...
        lab_rules.sort(key=lambda rule: (rule.get("group", ""), rule.get("priority", 99)))
        ...
        for patient in patients:
            ...
            aki = await self.engine._calc_aki_stage(patient, pid, his_pid)
            on_digoxin = await self.engine._has_recent_drug(pid, ["地高辛", "digoxin"], hours=72)
            ...
            for rule in lab_rules:
                ...
                correction_plan = self.engine._lab_correction_plan(
                    rule["test"],
                    value,
                    labs,
                    aki=aki,
        ...
        if triggered > 0:
            self.engine._log_info("检验预警", triggered)
