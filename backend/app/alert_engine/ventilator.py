...
class VentilatorMixin:
    ...
    async def scan_ventilator_weaning(self) -> None:
        ...
        for patient_doc in patients:
            ...
            active_vent_bind = await self._get_active_vent_bind(pid_str)
            device_id = active_vent_bind.get("deviceID") if active_vent_bind else await self._get_device_id_for_patient(patient_doc, ["vent"])
            cap = await self._get_latest_device_cap(device_id) if device_id else None

            if active_vent_bind and cap:
                risk = await self._build_weaning_recommendation(patient_doc=patient_doc, pid_str=pid_str, cap=cap, now=now)
                if risk:
                    await self._persist_weaning_assessment(
                        pid_str=pid_str,
                    ...
                    if isinstance(sbt, dict):
                        await self._persist_sbt_assessment(
                            pid_str=pid_str,
                            patient_doc=patient_doc,
                            now=now,
                            sbt=sbt,
            ...
            recent_extubation = await self._get_recent_extubation_bind(pid_str, now, hours=extub_hours)
            if recent_extubation and not active_vent_bind:
                ...
                accessory = await self._get_accessory_muscle_sign(pid, now, hours=12)
