...
class CardiacArrestRiskScanner(BaseScanner):
    ...
    async def scan(self) -> None:
        ...
        cfg = self.engine.config.yaml_cfg.get("alert_engine", {}).get("cardiac_arrest", {})
        ...
        for patient_doc in patients:
            ...
            temporal = await self.engine._latest_temporal_risk_record(pid_str, hours=6) if hasattr(self, "_latest_temporal_risk_record") else None
            ...
            hr_latest = self.engine._get_priority_param(latest_cap or {}, ["param_HR"]) if latest_cap else None
            sbp_latest = self.engine._get_sbp(latest_cap) if latest_cap else None
            map_latest = self.engine._get_map(latest_cap) if latest_cap else None
            ...
            qrs_records = await self.engine._get_qrs_records(pid, hours=48)
            ...
            alternating, alternating_meta = self.engine._detect_brady_tachy_alternating(hr_series)
            add_factor(
            ...
            hr_recent = await self.engine._get_param_series_by_pid(
                pid,
                "param_HR",
            ...
            hr_recent = self.engine._filter_series_quality("param_HR", hr_recent)
            sbp_recent = await self.engine._get_param_series_by_pid(
                pid,
                "param_nibp_s",
                now - timedelta(minutes=30),
                prefer_device_types=["monitor"],
            ...
            sbp_recent = self.engine._filter_series_quality("param_nibp_s", sbp_recent)
            if not sbp_recent:
                sbp_recent = await self.engine._get_param_series_by_pid(
                    pid,
                    "param_ibp_s",
                ...
                sbp_recent = self.engine._filter_series_quality("param_ibp_s", sbp_recent)
        ...
        if triggered > 0:
            self.engine._log_info("心脏骤停风险", triggered)
