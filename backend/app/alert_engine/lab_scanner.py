...
class LabScannerMixin:
    async def scan_lab_results(self) -> None:
        ...
        # group: 同组规则互斥，仅触发最高优先级（priority 数字越小越优先）
        # 规则按 group 内 priority 升序排列，命中后同 group 后续规则跳过
        lab_rules = [
            ...
            {"rule_id": "LAB_K_LOW", "test": "k", "name": "低钾血症", "condition": {"<": 3.0}, "severity": "warning", "group": "k_low", "priority": 2},
