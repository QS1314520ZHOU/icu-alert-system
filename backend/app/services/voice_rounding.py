...
from app.services.asr_client import ASRClient, ASREmptyTranscriptionError
...
SUSPECT_TYPE_LOW_CONF = "low_confidence"
SUSPECT_TYPE_UNIT = "unit_uncertain"
...
class VoiceRoundingService:
    ...
    async def transcribe(
        ...
        *,
        ...
        流程：
        1. 音频预处理（WebM/Opus → 16kHz mono WAV/PCM）
        2. ASR 转写
        3. 填充词清洗
        4. LLM 纠错 + 数值保护 + 药名保护
        ...
        """
        # 1. 音频预处理
        preprocessed: AudioPreprocessResult
        try:
            preprocessed = await preprocess_audio(
                audio_bytes,
                filename=filename,
                content_type=content_type,
                max_upload_bytes=int(self.cfg.get("max_upload_bytes", 50 * 1024 * 1024)),
                max_duration_seconds=float(self.cfg.get("max_audio_seconds", 300)),
                ffmpeg_timeout=float(self.cfg.get("ffmpeg_timeout", 60)),
            )
        except AudioPreprocessError:
            raise
        except Exception as exc:
            raise AudioPreprocessError(f"音频预处理失败: {exc}", status_code=500)

        # 2. ASR 转写（根据模式选择 wav_bytes 或 pcm_bytes）
        raw_text = await self.asr.transcribe(
            wav_bytes=preprocessed.wav_bytes,
            pcm_bytes=preprocessed.pcm_bytes,
        ...
        draft: dict[str, Any] = {
            ...
            "segments": [],  # 本阶段无时间戳，保留扩展位
            "processing": {
                "audio_normalized": True,
                "asr_mode": self.asr.mode,
                "llm_corrected": corrected.get("corrected", False),
                "source_format": preprocessed.source_format,
            },
    ...
    async def _write_edit_log(
        ...
        *,
        ...
        duration_seconds = (draft_doc or {}).get("duration_seconds", 0)
        ...
        # 计算修改字符数（审计用，不记录正文）
        edit_char_count = sum(
            len(e.get("before", "")) + len(e.get("after", ""))
            for e in edited_spans
        )
        ...
        log_doc: dict[str, Any] = {
            ...
            "edit_char_count": edit_char_count,
