...
DEBATE_SYSTEM = """你是ICU MDT辩论主持人。请只基于输入的专科意见、冲突和失败案例，输出JSON：
{"debate_summary":"string","revised_focus":["string"],"unresolved_conflicts":["string"]}"""


REFLECTION_SYSTEM = """你是ICU MDT反思审校员。请检查建议是否过度自信、证据不足或遗漏安全边界，输出JSON：
{"reflection_summary":"string","revisions":["string"],"confidence_adjustment":"up|same|down"}"""


SCHEMA_SELF_HEAL_SYSTEM = """你是JSON自修复器。把输入修复成目标schema兼容的JSON，只输出JSON，不增加医学内容。"""
...
def validate_specialist_output(agent_name: str, raw: dict[str, Any], valid_chunk_ids: set[str]) -> dict[str, Any]:
    ...
    if raw.get("priority") not in ("critical", "high", "medium", "low"):
        raw["priority"] = "medium"
    try:
        raw["confidence"] = max(0.0, min(float(raw.get("confidence", 0.6)), 1.0))
    except Exception:
        raw["confidence"] = 0.6
