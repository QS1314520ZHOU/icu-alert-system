...
class OutcomeInferenceWorker:
    """Batch worker that backfills alert_outcomes from persisted alerts."""
    ...
    async def seed_missing_outcomes(self, *, since_hours: int = 72, limit: int = 500) -> int:
        ...
        async for alert_doc in cursor:
            ...
            existing = await self.db.col("alert_outcomes").find_one({"alert_id": alert_id}, {"_id": 1})
            if existing:
                continue
    ...
    async def run_once(self, *, limit: int = 200, min_age_minutes: int = 30, seed_since_hours: int = 72) -> dict[str, Any]:
        started_at = datetime.now()
        seeded = await self.seed_missing_outcomes(since_hours=seed_since_hours, limit=limit * 3)
        ...
        cursor = self.db.col("alert_outcomes").find(query).sort("fired_at", 1).limit(max(int(limit or 200), 1))
        processed = 0
        missing_alerts = 0
        failed = 0
        for_update: list[str] = []
        async for outcome_doc in cursor:
            ...
            if not alert_doc:
                ...
                await self.db.col("alert_outcomes").update_one(
                    ...
                    {"$set": {"inference.status": "missing_alert", "inference.updated_at": datetime.now(), "updated_at": datetime.now()}},
                )
                continue
            try:
                ...
                failed += 1
                logger.exception("outcome inference failed alert_id=%s error=%s", alert_id, exc)
                await self.db.col("alert_outcomes").update_one(
                    {"_id": outcome_doc.get("_id")},
                    {"$set": {"inference.status": "failed", "inference.error": str(exc), "inference.updated_at": datetime.now(), "updated_at": datetime.now()}},
                )
